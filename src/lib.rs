...
use rocketsim::{
    ...
    CarInfo, CarState, DropshotInfo, GameMode, HeatseekerInfo, Mat3A, PhysState, Team,
    TileDamageState, TileStates, Vec3A, Vis, WheelPairConfig, consts,
};
...
impl ToFlat for RlviserMessage {
    ...
    fn to_flat(&self) -> Self::Flat {
        match self {
            ...
            Self::GameState(game_state) => fb::Message::GameState(Box::new((**game_state).clone())),
        }
    }
}
